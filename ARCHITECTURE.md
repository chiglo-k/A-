...
### Ingestion (Поступление данных)
    ...
    Спутниковые данные: Sentinel-1/2 (SAFE, JP2, GeoTIFF), GIBS (WMS/WFS/WMS-T слои);
    Прогнозные модели: GFS, Global Ocean Physics (NetCDF, GRIB2);
    Навигационные данные: AIS (JSON, CSV, бинарные потоки от провайдеров);
    Оперативные данные: Данные отчетов, фото/видео (PDF, DOCX, JPG, PNG);
    Открытые/ручные данные: Файлы CSV, XLSX, TXT, загружаемые пользователями.
